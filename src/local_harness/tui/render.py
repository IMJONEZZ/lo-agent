...
def set_palette(p: Palette) -> None:
    ...
    globals().update(
        PALETTE=p,
        JADE=p.jade, JADE_BRIGHT=p.jade_bright, JADE_DEEP=p.jade_deep,
        GOLD=p.gold, GOLD_DEEP=p.gold_deep, SAKURA=p.sakura, ROSE=p.rose,
        AMBER=p.amber, CREAM=p.cream, JADE_GREY=p.grey, JADE_MUTE=p.mute, INK=p.ink,
        CODE_THEME=p.code_theme,
        C_USER="bold " + p.jade, C_REASON="italic " + p.sakura, C_ANSWER=p.cream,
        C_TOOLMARK="bold " + p.gold, C_TOOL=p.gold, C_OK=p.jade, C_ERR=p.rose,
        C_RESAMPLE=p.amber, C_DIM=p.grey, C_SAKURA=p.sakura, C_GOLD=p.gold,
        B_OK=p.jade_deep, B_ERR=p.rose, B_INFO=p.info, B_ACCENT=p.gold_deep, B_SAKURA=p.sakura,
        _STATUS_STYLE={"running": p.gold, "completed": p.jade, "failed": p.rose},
        _SEP=("  │  ", p.grey),
        _CTX_COLORS={"system": p.jade_deep, "task": p.gold, "assistant": p.jade, "tool I/O": p.sakura},
    )


THEMES: dict[str, Palette] = {p.name: p for p in [
    ...
    Palette("osaka-jade", jade="#52cc9e", jade_bright="#74e6bd", jade_deep="#3a9d8c",
            gold="#dcbb7a", gold_deep="#c2a056", sakura="#e8a6c2", rose="#e6788c",
            amber="#e3a366", cream="#cfe0d6", grey="#7e978c", mute="#93a99c", ink="#101915",
            info="#64b4be", bg="#1a2722", surface="#21302a", panel="#283a32", code_theme="nord"),
    # Osaka Midnight — a deeper jade than the default (still soft, no black), dimmer.
    Palette("osaka-midnight", jade="#46c79b", jade_bright="#63e0b8", jade_deep="#2e8c7c",
            gold="#cfb074", gold_deep="#b08f44", sakura="#dca4bc", rose="#df6f86",
            amber="#d99c66", cream="#c3d4cb", grey="#6a8278", mute="#86998f", ink="#0a120e",
            info="#5aa6b0", bg="#121b17", surface="#18241f", panel="#1e2c26", code_theme="nord"),
    # Sakura — pink-forward on a soft dark plum (no black), accents eased.
    Palette("sakura", jade="#a0d4ad", jade_bright="#bce6c6", jade_deep="#6fae8c",
            gold="#e6c074", gold_deep="#d6a64e", sakura="#eda6c2", rose="#e87088",
            amber="#e8a36a", cream="#e8d5de", grey="#9a8490", mute="#bba6b2", ink="#1c1218",
            info="#7fb9c4", bg="#241a20", surface="#2c2028", panel="#342530", code_theme="material"),
    # Osaka Light — warm-paper background, dark jade text (needs dark=False).
    Palette("osaka-light", jade="#1f8a6b", jade_bright="#26a37f", jade_deep="#16705a",
            gold="#b07d1a", gold_deep="#8a5e10", sakura="#c25c86", rose="#c0344f",
            amber="#b5632a", cream="#2a2620", grey="#7a8278", mute="#5e655c", ink="#f3efe7",
            info="#2f7d8a", bg="#f3efe7", surface="#e9e3d6", panel="#e0d8c8",
            code_theme="friendly", dark=False),
    # Gruvbox — the classic warm retro palette (the harness's original look).
    Palette("gruvbox", jade="#b8bb26", jade_bright="#d3ff5e", jade_deep="#689d6a",
            gold="#fabd2f", gold_deep="#d79921", sakura="#d3869b", rose="#fb4934",
            amber="#fe8019", cream="#ebdbb2", grey="#928374", mute="#a89984", ink="#1d2021",
            info="#83a598", bg="#282828", surface="#32302f", panel="#3c3836", code_theme="gruvbox-dark"),
    # Catppuccin Mocha — soft pastel dark.
    Palette("catppuccin-mocha", jade="#a6e3a1", jade_bright="#b9f0b4", jade_deep="#94e2d5",
            gold="#f9e2af", gold_deep="#fab387", sakura="#f5c2e7", rose="#f38ba8",
            amber="#fab387", cream="#cdd6f4", grey="#6c7086", mute="#a6adc8", ink="#11111b",
            info="#89b4fa", bg="#1e1e2e", surface="#181825", panel="#252537", code_theme="material"),
    # Catppuccin Macchiato — a touch warmer/lighter than mocha.
    Palette("catppuccin-macchiato", jade="#a6da95", jade_bright="#b8e6aa", jade_deep="#8bd5ca",
            gold="#eed49f", gold_deep="#f5a97f", sakura="#f5bde6", rose="#ed8796",
            amber="#f5a97f", cream="#cad3f5", grey="#6e738d", mute="#b8c0e0", ink="#181926",
            info="#8aadf4", bg="#24273a", surface="#1e2030", panel="#2a2d44", code_theme="material"),
]}
...
def render_event(ev: Event) -> RenderableType | None:
    ...
    if ev.type == RUN_COMPLETED:
        return Panel(Text(p.get("answer") or ""), title="✓ completed",
                     title_align="left", border_style=B_OK)
    if ev.type == RUN_FAILED:
        return Panel(Text(p.get("error") or ""), title="✗ failed",
                     title_align="left", border_style=B_ERR)
    return None
...
def _model_call(p: dict) -> RenderableType:
    ...
    for tc in msg.get("tool_calls") or []:
        ...
        if body.plain:
            body.append("\n")
        body.append(
            f"→ {fn.get('name')}({_truncate(fn.get('arguments') or '', 120)})", style=C_TOOL
        )
    subtitle = Text(
    ...
    return Panel(body, title="assistant", title_align="left",
                 subtitle=subtitle, subtitle_align="right", border_style=B_INFO)
...
def _tool_call(p: dict) -> RenderableType:
    ...
    t = Text()
    t.append(f"🔧 {p.get('name')}({_truncate(p.get('arguments') or '', 100)}) → ", style=C_TOOL)
...
def _guardrail(p: dict) -> RenderableType | None:
    ...
    if p.get("rescued"):
        names = ", ".join(
            tc.get("function", {}).get("name", "?") for tc in p.get("rescued_calls") or []
        )
...
def chat_badges(p: dict) -> Text:
    ...
    if s and s.get("mean_logprob") is not None:
        ...
        bits.append((f"logprob {s['mean_logprob']:+.2f}", confidence_style(s["mean_logprob"])))
    if p.get("timing_ms"):
...
def confidence_text(lp_content: list) -> Text:
    ...
    for tok in lp_content:
        token = tok.get("token", "") if isinstance(tok, dict) else getattr(tok, "token", "")
        lp = tok.get("logprob", 0.0) if isinstance(tok, dict) else getattr(tok, "logprob", 0.0)
        if lp > -0.3:
...
_LANG_BY_EXT = {
    ...
    "md": "markdown", "rst": "rst", "sql": "sql", "lua": "lua", "kt": "kotlin",
    "swift": "swift", "r": "r", "jl": "julia", "ex": "elixir", "exs": "elixir",
    "vim": "vim", "diff": "diff", "patch": "diff", "xml": "xml", "proto": "proto",
}
...
def code_artifact(code: str, lang: str = "text", *, title=None, subtitle=None,
                  border: str = B_INFO, line_numbers: bool = True,
                  start_line: int = 1, max_lines: int | None = None) -> RenderableType:
    ...
    try:
        body: RenderableType = Syntax(
            shown, lang, theme=CODE_THEME, line_numbers=line_numbers,
            word_wrap=True, start_line=start_line, background_color="default")
    except Exception:
        body = Text(shown, style=C_ANSWER)
    if note:
        body = Group(body, Text("  " + note, style=C_DIM))
    return Panel(body, title=title, title_align="left", subtitle=subtitle,
                 subtitle_align="right", border_style=border, padding=(0, 1))


def _artifact_title(glyph: str, path: str, *, bg: str) -> Text:
    return Text.assemble((f" {glyph} ", f"bold {INK} on {bg}"), ("  ", ""),
                         (path, "bold " + GOLD))


def file_artifact(path: str, content: str, *, created: bool = False,
                  max_lines: int = 60) -> RenderableType:
    ...
    return code_artifact(content, lang, title=title, subtitle=sub, border=B_OK,
                         max_lines=max_lines)
...
def diff_artifact(path: str, old: str, new: str) -> RenderableType:
    ...
    import difflib
    rows = list(difflib.unified_diff(old.splitlines(), new.splitlines(),
                                     lineterm="", n=2))
    body = Text()
    ...
    return Panel(body if body.plain else Text("(no textual change)", style=C_DIM),
                 title=title, title_align="left", subtitle=sub, subtitle_align="right",
                 border_style=B_ACCENT, padding=(0, 1))


def plan_artifact(text: str, *, title: str = "plan", subtitle: str | None = None) -> RenderableType:
    ...
    return Panel(_markdown(text.strip()),
                 title=Text.assemble((f" ✦ {title} ", f"bold {INK} on {SAKURA}")),
                 title_align="left",
                 subtitle=Text(subtitle, style=C_DIM) if subtitle else None,
                 subtitle_align="right", border_style=B_SAKURA, padding=(1, 1))


def run_output_artifact(command: str, output: str, *, ok: bool = True,
                        lang: str = "text") -> RenderableType:
    ...
    title = Text.assemble((f" {glyph} ", f"bold {INK} on {bg}"), ("  ", ""),
                          (_truncate(command, 80), C_DIM))
    return code_artifact(output or "(no output)", lang, title=title,
                         border=B_OK if ok else B_ERR, line_numbers=False, max_lines=80)
...
def chat_tool(p: dict) -> RenderableType:
    ...
    if name == "run_code":
        try:
            ...
            code = raw_args
        head = Text.assemble(("  ⚡ ", C_TOOLMARK), ("code-mode", C_TOOL),
                             (" → ", C_DIM), (_truncate(result, 400), C_ERR if failed else C_OK))
        if code.strip():
            art = code_artifact(code, "python", border=B_INFO, max_lines=24,
    ...
    head = Text.assemble(
        ("  ⚙ ", C_TOOLMARK),
        (f"{name}(", C_TOOL), (short, C_ANSWER), (")", C_TOOL),
        (" → ", C_DIM),
    ...
    if name == "write_file":
        return Group(head, file_artifact(path, parsed.get("content") or "",
                                         created=True))
    if name == "edit_file":
        return Group(head, diff_artifact(path, parsed.get("old_string") or "",
                                         parsed.get("new_string") or ""))
    if name == "read_file":
        # the result IS the file content — highlight it by extension
        return Group(head, code_artifact(result, lang_for_path(path),
                                         title=_artifact_title("⊙ read", path, bg=JADE_DEEP),
                                         border=B_INFO, max_lines=60))
    return head
...
def welcome_panel(model: str, caps) -> RenderableType:
    ...
    body.append("   ·  /help commands  ·  ^o connect  ·  ^t history  ·  Esc stops a turn",
                "dim")
    return Panel(body, title="✦ local_harness", title_align="left",
                 border_style=B_ACCENT, padding=(1, 3))
...
def status_bar(*, preset: str, tier: int, glyphs: list[str], saved: float,
               deterministic: bool, learn: str,
               ctx: tuple[str, float | None] | None = None,
               vim: str | None = None) -> Text:
    ...
    if ctx is not None:
        label, frac = ctx
        color = C_OK if (frac is None or frac < 0.70) else (C_TOOL if frac < 0.85 else C_ERR)
        out.append(*_SEP)
...
def banner_body(model: str, caps, *, tools: list[str], skills: list[str],
                memory_summary: str, preset_name: str, preset_blurb: str) -> RenderableType:
    ...
    rows.append(_row(f"skills ({len(skills)})",
                     " ".join(sorted(skills)) if skills else "none", C_USER if skills else C_DIM))
    if memory_summary:
...
def usage_panel(stats: dict, *, model: str, tier: int, deterministic: bool,
                learn_summary: str | None) -> RenderableType:
    ...
    body.append(f"   ~{_money(saved)} the same {toks:,} tokens would cost on an "
                f"Opus-class API\n", style=C_DIM)
    body.append(f"  across {calls} model call(s) — local marginal cost is zero, so "
                "best-of-N and background learning are free.\n\n", style=C_DIM)

    body.append("determinism\n", style="bold")
    if deterministic:
        body.append("  ⟲ bit-exact & replayable", style=C_OK)
        body.append("   every call logs its seed; ^r re-runs identically.\n\n", style=C_DIM)
    else:
        body.append("  ⟲ best-effort", style=C_TOOL)
        body.append("   this server doesn't pin a seed — replay may drift.\n\n", style=C_DIM)
    ...
    if sum(bands) and stats.get("mean_lp") is not None:
        ...
        body.append(f"  mean logprob {stats['mean_lp']:+.2f}\n",
                    style=confidence_style(stats["mean_lp"]))
        body.append(f"  {bands[0]} expected · {bands[1]} surprising · {bands[2]} very-surprising "
                    "tokens\n", style=C_DIM)
        body.append("  surface-form only — NOT a correctness signal. Real uncertainty comes "
    ...
    body.append(f"  {learn_summary or 'idle — consolidation runs free between turns.'}\n",
                style=C_OK if learn_summary else C_DIM)
    return Panel(body, title="✦ usage & advantages", title_align="left",
                 subtitle=f"{model} · tier {tier}", subtitle_align="right",
                 border_style=B_ACCENT, padding=(1, 2))
...
def context_breakdown(messages: list[dict], tools: list[dict] | None = None) -> list[tuple[str, int]]:
...
def context_panel(breakdown: list[tuple[str, int]], *, window: int | None,
                  trigger_frac: float = 0.85, width: int = 44) -> RenderableType:
    ...
    if window:
        ...
        parts.append(Text.assemble(
            (f"{_ktok(used)} ", "bold " + C_ANSWER), (f"/ {_ktok(window)} ", C_DIM),
            (f"({pct}%)", "bold " + (C_OK if pct < 70 else C_TOOL if pct < 85 else C_ERR))))
        ...
        parts.append(bar)
        parts.append(Text.assemble(
            (" " * trig, ""), ("↑ auto-compaction at ", C_DIM),
            (f"{trigger_frac:.0%} ", "bold " + C_SAKURA),
            (f"({_ktok(int(window * trigger_frac))})", C_DIM)))
    else:
        parts.append(Text.assemble((f"{_ktok(used)} used", "bold " + C_ANSWER),
    ...
    parts.append(Text(""))
    parts.append(legend)
    return Panel(Group(*parts), title="✦ context", title_align="left",
                 border_style=B_ACCENT, padding=(1, 2))


def memory_panel(sections: list[tuple[str, str]],
                 recall: list[tuple[str, str]] | None = None) -> RenderableType:
    ...
    for title, content in sections:
        ...
        text = content.strip()
        body.append(Padding(Text(text or "(empty)", style=C_ANSWER if text else C_DIM),
                            (0, 0, 1, 2)))
    if recall:
        body.append(Text("relevant past runs (recall)", style="bold " + C_SAKURA))
        for kind, text in recall:
            body.append(Padding(Text.assemble((f"[{kind}] ", C_DIM), (text, C_ANSWER)),
                                (0, 0, 0, 2)))
    return Panel(Group(*body), title="✦ memory", title_align="left",
                 border_style=B_ACCENT, padding=(1, 2))
...
def findings_panel(text: str, kind: str) -> RenderableType:
    ...
    sev_color = {"blocker": ROSE, "critical": ROSE, "major": AMBER, "high": AMBER,
                 "minor": GOLD, "medium": GOLD, "nit": JADE_GREY, "low": JADE_GREY}
    body = Text()
    ...
    return Panel(body, title=title + "  (grammar-validated)", title_align="left",
                 border_style=B_ACCENT, padding=(1, 2))
...
def plan_fork_panel(candidates: list) -> RenderableType:
    ...
    parts: list[RenderableType] = [
        Text(f"forked {len(candidates)} candidate plans, ranked by a verifier "
             "(free on the local prefix-cache):", style=C_DIM)
    ]
    for i, c in enumerate(candidates):
        ...
        if i == 0:
            parts.append(Text.assemble(("\n★ chosen", "bold " + C_OK),
                                       (f"   score {score:+.2f}", C_DIM)))
            parts.append(plan_artifact(text, title="winning plan",
                                       subtitle=f"score {score:+.2f}"))
        else:
            parts.append(Text.assemble((f"\n  #{i + 1}", "bold " + C_DIM),
                                       (f"  score {score:+.2f}", C_DIM)))
            parts.append(Padding(Text(_truncate(text, 240), style=C_DIM), (0, 0, 0, 4)))
    return Panel(Group(*parts), title="⑂ plan-fork", title_align="left",
                 border_style=B_OK, padding=(1, 2))
...
def chat_render_event(ev: Event, show_thinking: bool = True) -> RenderableType | None:
    ...
    if ev.type == TOOL_CALL:
        return chat_tool(p)
    if ev.type == POLICY_TRIGGERED:
        return Text.assemble(("  ↻ ", C_RESAMPLE),
