...
reviews and promotes it. Enabled only via `harness background --autonomous-actions`.
"""
...
async def propose_actions(log: EventLog, client: OpenAICompatClient, drafts_dir,
    ...
    for run in log.runs():
        ...
        resp = await client.chat(GenerationRequest(
            messages=[Message(role="user", content=(
                ...
                f"Task: {run.task}\nTools used: {tools}\nFailure: {err[:300]}"))],
            sampling=SamplingParams(temperature=0.3, max_tokens=256, seed=1, extra=dict(NO_THINK))))
