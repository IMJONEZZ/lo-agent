"""`harness bench` — run the harness's claimed advantages as actual measurements.
...
server can't do it — no green checkmark is ever printed for something we didn't run.
    ...
    uv run harness bench --url http://HOST:PORT [--n 8]
...
async def bench_determinism(client: OpenAICompatClient, caps: Capabilities) -> BenchResult:
    if not caps.seed:
        return BenchResult("determinism", "SKIP", "server doesn't honor a seed",
    ...
    if a.strip() and a == b:
        return BenchResult("determinism", "PASS", "2 seeded runs bit-identical",
                           "API: nondeterministic (batch-variant kernels, no real seed)")
    return BenchResult("determinism", "FAIL", "seeded runs diverged",
                       "API: also nondeterministic")


async def bench_batch_invariance(client: OpenAICompatClient, caps: Capabilities,
                                 *, concurrency: int = 7) -> BenchResult:
    ...
    from `harness bench` (opt-in), never at connect-time."""
    if not caps.seed:
        return BenchResult("batch-invariance", "SKIP", "server doesn't honor a seed",
    ...
    if ok is None:
        return BenchResult("batch-invariance", "SKIP", "probe inconclusive (request error)", "")
    if ok:
        return BenchResult("batch-invariance", "PASS",
                           f"output unchanged under {concurrency}-way concurrent load",
                           "API: batch-variant kernels — output drifts with server load")
    return BenchResult("batch-invariance", "FAIL",
                       f"output changed under {concurrency}-way load (batch-variant)",
                       "API: same flaw, but you can't detect it (no seed, no replay)")


async def bench_grammar(client: OpenAICompatClient, caps: Capabilities,
                        skills_dir: str, n: int) -> BenchResult:
    if caps.grammar is None:
        return BenchResult("grammar", "SKIP", "no grammar/guided decoding on this server",
    ...
    return BenchResult("grammar", status, f"{valid}/{n} valid by construction",
                       "API: structured output can still violate hard grammars")


async def bench_uncertainty(client: OpenAICompatClient, caps: Capabilities,
                            n: int) -> BenchResult:
    ...
    _, agree_clear = await self_consistency(
        client, caps, clear, n=n, sampling=SamplingParams(temperature=1.0, max_tokens=16))
    _, agree_open = await self_consistency(
        client, caps, openq, n=n, sampling=SamplingParams(temperature=1.0, max_tokens=32))
    ok = agree_clear >= agree_open
    return BenchResult(
        "uncertainty", "PASS" if ok else "INFO",
...
async def bench_semantic_entropy(client: OpenAICompatClient, caps: Capabilities,
                                 n: int) -> BenchResult:
    ...
    determinate = [Message(role="user", content="What is the capital of France? Answer in one word.")]
    openq = [Message(role="user", content="Name an interesting historical figure. Reply with just the name.")]
    ...
    judge = "" if det.judged and opn.judged else " (lexical fallback — judge unavailable)"
    return BenchResult(
        "semantic-entropy", "PASS" if ok else "INFO",
...
def bench_cost(capture: str | Path = "demos/frontier_cost_capture.json") -> BenchResult:
    ...
    if p.exists():
        try:
            ...
            pass
    return BenchResult("cost", "INFO", "$0.00 local marginal cost",
                       f"frontier: {frontier}")


async def run_bench(client: OpenAICompatClient, caps: Capabilities,
                    skills_dir: str = "skills", n: int = 8,
                    batch_invariance: bool = True) -> list[BenchResult]:
...
def format_report(results: list[BenchResult], *, model: str, server: str, tier: int) -> str:
    lines = ["local_harness bench — advantages as measurements",
             f"server: {server} · tier {tier} · {model}", ""]
    for r in results:
        lines.append(f"  {_MARK.get(r.status, '?')} {r.name:<13} {r.status:<5} {r.detail}")
        if r.frontier:
